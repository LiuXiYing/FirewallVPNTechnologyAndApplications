...
1. DNS 的作用是什么？为什么访问网站时通常要先进行 DNS 解析？
   ...
   > 答：DNS 的作用：DNS（域名系统）是互联网的 “电话簿”，负责将人类易读的域名（如 www.baidu.com）转换为计算机可识别的 IP 地址（如 183.2.172.17），实现域名到网络地址的映射。
访问网站先做 DNS 解析的原因：计算机网络通信依赖 IP 地址定位主机，而人类难以记忆纯数字的 IP 地址。DNS 解析让用户可以使用简洁的域名访问网站，同时让服务器可以灵活变更 IP 而不影响用户访问。

2. 你抓到的 DNS 请求包和响应包中的 **Transaction ID** 是否一致？这个字段有什么作用？

   > 答：一致性：请求包（No.69）和响应包（No.83）的 Transaction ID 均为 0x6f4c，完全一致。
作用：Transaction ID 是 DNS 协议中用于匹配请求与响应的唯一标识。客户端发起请求时生成一个随机 ID，服务器响应时必须携带相同 ID，确保客户端能将收到的响应正确对应到之前的请求，避免网络中多个 DNS 交互发生混淆。


3. 这次实验中查询类型通常是 `A` 记录。`A` 记录表示什么含义？

   > 答：A 记录（Address Record）是 DNS 中最基础的记录类型，用于将域名直接映射到 IPv4 地址，告诉客户端 “这个域名对应的服务器 IPv4 地址是什么”，是访问网站时最常用的 DNS 查询类型。

4. DNS 默认通常使用哪个传输层协议和端口？在什么情况下可能会改用 TCP？

   > 答：默认配置：DNS 默认使用 UDP 协议，端口为 53，适合传输短小的查询 / 响应数据，效率更高。
改用 TCP 的场景：
当 DNS 响应数据超过 512 字节时（UDP 报文最大限制），会触发 TCP 传输；
区域传送（Zone Transfer）时，DNS 服务器之间同步完整域名数据必须使用 TCP；
现代 DNSSEC（安全扩展）场景下，为保证数据完整性和安全性，也会优先使用 TCP。

5. 为什么说 `gethostbyname()` 不是现代工程中推荐继续使用的接口？请至少写出两点原因。

   > 答：不支持 IPv6：gethostbyname() 只能返回 IPv4 地址，无法处理 IPv6 网络环境，不符合现代互联网双栈（IPv4/IPv6）的发展趋势。
线程不安全：该接口返回的静态缓冲区在多线程环境下会被共享，容易导致数据竞争和结果错乱，现代编程更推荐线程安全的替代接口（如 getaddrinfo()）。
功能局限：无法获取更丰富的地址信息（如端口、协议族），也不支持 DNS 解析的高级配置（如超时、重试），扩展性差。

6. 如果一个域名返回了多个 IP 地址，这通常说明了什么？

   > 答：负载均衡：将用户请求分发到不同服务器，避免单台机器过载，提升服务吞吐量；
高可用：某台服务器故障时，其他 IP 对应的服务器仍可提供服务，保证网站不间断访问；
就近访问（CDN / 智能 DNS）：根据用户地理位置返回最近的服务器 IP，降低网络延迟，提升访问速度。
