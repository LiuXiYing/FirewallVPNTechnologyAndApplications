...
1. DNS 的作用是什么？为什么访问网站时通常要先进行 DNS 解析？
   ...
   > 答：DNS（域名服务器）的作用就是将人类可读的名称转换为机器识别的IP地址，供计算机相互连接；计算机网络通信依赖IP而非域名，DNS就是负责将域名转换为IP辅助通信。
...
3. 这次实验中查询类型通常是 `A` 记录。`A` 记录表示什么含义？
   ...
   > 答：告诉DNS服务器查询的情况是将域名转换为一个IPv4的IP地址进行返回。
...
6. 如果一个域名返回了多个 IP 地址，这通常说明了什么？
   ...
   > 答：该域名下的服务采用了负载均衡、高可用性（灾备）或内容分发网络（CDN）技术。通过这种方式，DNS 服务器将用户流量分散到多台服务器上，从而提高网站的整体处理能力、访问速度和抗风险能力。
